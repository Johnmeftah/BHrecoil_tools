...
def match_mergers():
    ...
    for row in mergers_data:
        ...
        matched_mergers.append({'bh1': bh1, 'bh2': bh2, 'merger_z': merger_z, 'snapshots': files})
    return matched_mergers


# phase II
# BHs def
def find_BH(snapshot):
    BH_filter = pn.filt.LowPass('tform', 0.0)
    BHs = snapshot.stars[BH_filter]
    return BHs
...
def track_BHs(match):
    ...
    for step in match['snapshots']:
        ...
        for i in range(len(BHs)):
            ...
            if bh_id == match['bh1'] or bh_id == match['bh2']:
